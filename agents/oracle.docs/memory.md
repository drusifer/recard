...
- `agents/`: Contains persona-specific documentation and state.
  ...
  (current: `CHAT_SPRINT_1_2.md`; Sprint 3 still in the live `CHAT.md` as
  of this groom).
- `docs/`: Global documentation (PRD.md, ARCHITECTURE.md, DECISIONS.md, USER_STORIES.md).
- `task.md`: Single source of truth for the current sprint (maintained by Mouse). Sprint 1 + Sprint 2 both DONE; Sprint 3 implementation (Phases 12-19) DONE as of 2026-08-15, Stage 3 close-out in progress.
- `src/`: app source — deck.js, state.js (reducer + viewFor redaction),
  protocol.js, session.js, ui.js, qrcode.js, presets.js, rulesReference.js,
  handOrder.js (v1.2, D14), main.js.
- `tests/`: unit tests (`*.test.js`, run via `npm test`, 64 passing) +
  `e2e.smoke.mjs` (`npm run test:e2e`, real 2-browser Playwright/WebRTC;
  now also covers zones, DEAL_MORE, pass marker, hand-sort persistence,
  and cursor broadcast).
