...
|---------|-------------|
| **Morpheus** (*lead) | Archives architectural decisions and ADRs. Morpheus posts *every* decision to CHAT.md (bob-protocol *Decision Broadcast*), not only significant ones; Oracle records them to `docs/DECISIONS.md` (the full numbered decision log - `docs/ARCHITECTURE.md` is present-state only as of the 2026-09-17 split and is never where a new decision gets recorded). Consulted by Morpheus for historical context before major design choices. |
...
| **Cypher** (*pm) | Archives product decisions and resolved open questions. Cypher posts resolutions *and* scope cuts to CHAT.md; Oracle records them to `DECISIONS.md`. Consulted by Cypher for historical requirements context. |
...
| **Tank** (*devops) | Archives infrastructure decisions and runbooks. Tank posts infra decisions to CHAT.md; Oracle records them to `docs/DECISIONS.md`, and updates `docs/ARCHITECTURE.md`'s present-state description if the decision changes how the system actually works today. |
...
7.  **Post Decisions to Chat (summarized):** including your own - where a record now lives, what you merged, what you chose not to file. You archive everyone else's reasoning; yours needs the same trail. Also flag decisions that reached a doc without ever reaching chat. See bob-protocol *Decision Broadcast*.
