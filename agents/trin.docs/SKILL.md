...
*   **`*qa judge` uses real tool-call data, not CHAT.md**: run `python3 agents/tools/trace_annotate.py [--date YYYY-MM-DD]` to get a ground-truth trace of actual tool/skill invocations from the real Claude Code JSONL session transcripts. There is no `make judge-trace` wrapper — no bob-protocol project checked (6 as of 2026-08-15) has that target wired, so don't go looking for it; invoke the script directly. If `agents/tools/trace_annotate.py`/`trace_rules.json` are missing from this project, copy them from any sibling bob-protocol project (generic, project-agnostic). CHAT.md is a prose summary personas write about their own work — it cannot show tool-call-level behavior and will make every judge run look better than it was. See `agents/skills/judge/SKILL.md` Step 1.
