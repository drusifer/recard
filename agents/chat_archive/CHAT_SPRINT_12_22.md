# CHAT_SPRINT_12_22 — Sprint Archive

## Summary

Sprint 12 ("piles are the interaction", v2.0): every pile hovers/taps to a fixed anchor popover instead of scattered buttons; Draw drags AND taps (Gate 1), reveal becomes a tap on the card. Sprint 13 (v3.1, D42): Pile becomes a real per-type module (visibility/redactCard/dropRule/cardActions/pileActions), zero user-visible change, exhaustively characterization-tested. Sprint 14 (D43): write-side (reducer dispatch) wired through the same per-type modules; DECISIONS.md's D20 ceiling formally superseded by ARCHITECTURE.md. Sprint 20 (D50, US-54): Add Zone hides itself when GameConfig disallows player zones. e2e-fix-for-D52: full e2e migration to the radial-menu DOM, 5 real bugs found+fixed, 2 minor visual findings backlogged. Sprint 22 (D53, US-56..59): `dropRule` enum retired for real per-pile `canAccept`/`resolveDropTarget` polymorphism; three new pile kinds (foundation/cascade/rankAdjacent) proven against Solitaire and Spit; `GameConfig.zones` lets presets auto-build a table. D53 follow-up: Gin Rummy gets a real declared discard zone; configured-zone naming fixed project-wide. Then a long ad-hoc post-sprint session (usesMiddle retired; card/UX follow-ups; panel move+resize, made local not replicated; DOM flattened to `table-surface -> #zones -> .zone`) drove Zone and Pile apart into genuine, separate Web Components — ending with six components (`score-zone`/`deck-zone`(later retired)/`zone-panel`/`seat-zone`(later retired)/`fan-pile`/`header-actions`), Piles-in-a-Zone grouping (`<table-zone>`, later generalized), and the Table Zone density/overlap chase that flows into the still-open work archived at the top of the live `CHAT.md`.
