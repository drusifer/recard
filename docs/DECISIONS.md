...
The full, continuously-numbered decision log (D1 onward). **The highest
number is always the current binding state** for whatever it touches —
a later decision can extend, narrow, or outright reverse an earlier one
(e.g. D83/D84 remove the per-viewer redaction D7 introduced; D93
declares "no canonical piles," superseding earlier per-kind special-
casing; D129 deletes four CSS overlap formulas D118-D125 had added two
days earlier). Where that happened, the superseding decision usually
says so explicitly — read newest-first for the section you care about.

**Structure:** decisions from D82 through the current highest number are
newest-first, each its own `### D<n>.` entry. Below that (starting at
D81 going down to D1) is a legacy block, forward-chronological, grouped
under its original per-version `## vX.Y Decisions` headings — a
convention from before continuous newest-first numbering started; not
reordered, since rewriting a large, stable, heavily-cross-referenced
part of the project's history has no benefit. `tools/checkDecisionOrder.mjs`
(`make check-decisions`) checks this file for duplicate D-numbers and
that the modern section stays newest-first; it auto-detects the legacy
boundary and does not check ordering below it.

**For present-state architecture** (how the system works today, not the
history of how it got there), see `docs/ARCHITECTURE.md` and the docs it
links to. This file is the historical record — a decision's own
rationale/consequences prose is preserved here verbatim rather than
summarized, even where the outcome it describes was later superseded;
that's what makes "why is it like this" answerable later.

## Topic index

A table of contents by theme, not a summary — every entry below just
points at its own `### D<n>.` heading further down this same file. A
decision appears under more than one theme where it genuinely spans
both (e.g. D129 is both domain-model and camera-adjacent); nothing here
is single-bucketed just to keep the list tidy.
...
D7: middle cards generalize redaction · D8: PLAY/REVEAL/PICKUP actions · D82: Core invariant enforced (MeldPile/ExilePile reopened) · D83: no per-card ownership/visibility gate either · D84: card redaction removed entirely · D85: bulk pile actions made fully permissive

**Drag-and-drop & card actions** — the core invariant, the universal
DnD guarantee, context-menu actions, drop targets
D19: live card-drag broadcast · D28: touch drag is a pointer-driven recognizer · D34: pile-level actions, fixed anchor · D35: dragging a pile-level action · D36: single-legal-target tap shortcut · D51: bigger cards, Actionable interface unified · D52: radial menu, click-to-follow · D64: deck reparentable between Zones · D65: Draw's drag-to-pick-up fixed · D66: Draw draggable from the deck stack · D67: deck top card is a real draggable card · D68: live drag relative to the dragging player's seat · D96: universal DnD guarantee, structural test · D101: card actions via right-click context menu · D102: `play` retired — a card leaving a hand is `MOVE_CARD` · D103: `REVEAL` becomes `FLIP_CARD` · D119: a card's lower half is a "column" drop target · D121: fourth "adjacent" drop target, near/far halo · D122: Move reveals real, card-relative drop targets · D123: one ghost-card drop preview · D124: "adjacent" reachable BETWEEN two placed cards

**Domain model** — the Zone/Pile split, the class hierarchy,
Pileable/Stackable/Stack, "no canonical piles," reducer dispatch
D12: `table` generalizes to `zones` · D17: personal per-seat zones · D18: seating is per-viewer client-side presentation · D21: stack/overlap as a `layout` field · D23: unified `Pile` storage model · D38: four primitives — GameConfig/DeckDefinition/Zone/Pile · D39: Pile is a five-method interface · D41: D39 split into two tranches · D42: pile-type module contract · D43: transfer-shaped actions dispatch through the Pile write side · D44: `reduce()`'s switch becomes an object-map registry · D45 / D45 (continued): `discard` is the first post-D42 Pile type · D46: `GameConfig.allowsPlayerZones` · D50: `viewFor` carries `gameConfig.allowsPlayerZones` · D53 / D53 follow-up: `dropRule` retired for real polymorphism · D54: Zone and Pile become real, separate Web Components · D55: Zone becomes a real, declaratively-configured entity · D56: Pile/Zone become real ES class hierarchies · D57: Piles/Zones/cards are all genuinely Movable · D62: `REMOVE_ZONE`/`REMOVE_PILE` · D70: default numbered kind name · D72: default pile name is "Pile" not "Zone" · D73: Zone headings never suppressed · D75: `CREATE_ZONE`/ungroup unified on `makeStandaloneZone` · D86: `CHANGE_PILE_TYPE` fixed · D87: `CHANGE_PILE_TYPE` full symmetry · D89: orphaned hand piles structurally impossible · D90: "zone" never means "pile" · D93: THERE SHOULD BE NO CANONICAL PILES · D94: `viewFor` polymorphic dispatch · D97: HandPile splits into Player/OpponentHandPile · D107: `Pileable` — the type of a thing in a pile · D116: `GroupedPile` · D129: `Stack`/`Stackable`

**Pile actions & controls** — split/pickup, merge, spread/tighten-
loosen, change-pile-type, hand tools
D14: hand order persists client-side only · D15: incremental dealing (`DEAL_MORE`) · D16: pass marker reuses existing authorization · D25: pile types declare their own actions · D29: dealing is a pile-level action · D63: `changePileType` (zone⇄discard) · D71: `changePileType` widens to a 5-kind cycle · D74: `pass` removed outright · D92: Split/Pickup guided fan-and-choose-a-gap picker · D95: card-count badges, universal · D98: `MERGE_PILE` · D99: hand size default from preset data · D104: card actions get a browser test layer · D106: pile spread, Tighten/Loosen · D112: a hand is a pile you can move/merge like any other · D118: shrink-to-fit pile header · D120: Battlefield gets Tighten/Loosen
