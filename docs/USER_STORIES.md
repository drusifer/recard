...
### US-131: Every Jev game has a README with its statechart
**As** someone reading or adding a Jev game, **I want** a README per game
with a description and a diagram of the bot's turn, **so that** I can see
how it plays without reading YAML - and it cannot go stale.

**AC:**
1. `games/<game>/README.md` exists for every game in `tools/jev/games.mjs`,
   generated by `bobp make jev-readme`; a test fails when one is stale.
2. The diagram is a Mermaid `stateDiagram-v2` built from the COMPILED
   machine with `@xstate/graph` (`toDirectedGraph`) - not a second parse of
   `turn.yaml`. The one gap (eventless `always` edges, which the library
   omits) is read from the same machine's state nodes.
3. `safe`, `move` and `busy` states are marked. The reserved `left` state
   and the QUIT edges into it are described in prose, not drawn.
4. Descriptions live in `turn.yaml` (`description:` on the file and on
   every state); a state with none fails a test.
5. The README also lists the table setup, the players and the questions,
   all from the game's own files.
